...
%% Hardware ----
% monitor ---
monitor.gamma = 1; %2.2; % used for linearizaion of contrast
...
prestim(1).orient = [45 -45]; % degrees clockwise
prestim(1).driftspeed = [0 0]; % deg/s positive is rightward
...
% [60/(sf*speed) = integer]
prestim(1).transient.contrastincr = 0.6;
prestim(1).transient.stim = 1;
prestim(1).transient.duration = 0.200;
prestim(1).transient.timewindow = [-2 -1]; % relative to end of prestim
prestim(1).instruct = [];
prestim(1).quest = 'Left or Right';
...
prestim(2).contrast = 0.2;
prestim(2).contrastbin = true;
prestim(2).color = [0]; % leave empty for luminance contrast
prestim(2).driftspeed = [-1.5 0; 1.5 0]; % deg/s positive is rightward
prestim(2).transient.contrastincr = 0.6;
prestim(2).transient.stim = 1;
prestim(2).transient.duration = 0.200; % pick an integer number of frames in s
prestim(2).transient.timewindow = [-2 -1]; % relative to end of prestim
prestim(2).instruct = [];
prestim(2).quest = 'Left or Right';
...
prestim(3).orientations = [-45 -45; 45 45]; % degrees clockwise [start end]
prestim(3).driftspeed = [-1.5 1.5]; % deg/s positive is rightward
prestim(3).trackstim = 1; % 
prestim(3).change.degpersec = 2; % deg/f (choose integer frame counts
prestim(3).change.interval = [0.5 1.5]; 
prestim(3).change.prob = 0.1;
...
prestim(4).color = [1 0 0;0 1 0]; % leave empty for luminance contrast
prestim(4).orientations = [-45 -45; 45 45]; % degrees clockwise [start end]
prestim(4).driftspeed = [-1.5 0; 1.5 0]; % deg/s positive is rightward
