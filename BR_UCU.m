...
try
    ...
    PsychDefaultSetup(2);
    HideCursor; ListenChar(2); % silence keyboard for matlab
    ...
    if Debug % if in debug mode, only open a 75% screen
        ...
        WindowRect = ...
            [1080 0 1080+1920 1080]; %#ok<*UNRCH> %debug
    else
    ...
    [monitor.w, monitor.wrect] = ...
        ...
        WindowRect, [], 2, stereoMode); %#ok<*NODEF>
    ...
    % Log init
    log.ev = table('VariableNames',{'t','type','info'});
    ...
    if eyetracker.do
        ...
        log.ev.t = [log.ev.t; GetSecs];
        log.ev.type = [log.ev.type; 'EyeStart'];
        log.ev.info = [log.ev.info; 'GazeBuffer'];
    end
    ...
    % Framerect =====================
    bg.align.Frame.SizePix = round(...
        bg.align.Frame.Size .* bg.align.Screen.Deg2Pix);
    ...
    while b <= length(expt.blocks) && ~StopExp
        ...
        log.ev.t = [log.ev.t; vbl];
        log.ev.type = [log.ev.type; 'ExpStart'];
        log.ev.info = [log.ev.info; 'Instruction'];
        if eyetracker.do
        ...
        % wait for key
        [secs, ~, ~] = KbWait();
        log.ev.t = [log.ev.t; secs];
        log.ev.type = [log.ev.type; 'KeyStart'];
        log.ev.info = [log.ev.info; 'none'];
        if eyetracker.do
        ...
        while t <= length(TRIALS) && ~StopExp
            ...
            log.ev.t = [log.ev.t; vbl];
            log.ev.type = [log.ev.type; 'FixStart'];
            log.ev.info = [log.ev.info; 'none'];
            if eyetracker.do
            ...
            while (GetSecs-FixT0) < trialtime.FixT && ~StopExp
                ...
                if strcmp(KbName(find(keyCode)),keys.esc)
                    StopExp = true;
                    log.ev.t = [log.ev.t; secs];
                    log.ev.type = [log.ev.type; 'KeyStop'];
                    log.ev.info = [log.ev.info; 'none'];
                    if eyetracker.do
            ...
            if trialtime.PrestimT % only if a prestim phase is set
                ...
                while (vbl - PreStimT0) < trialtime.PrestimT && ~StopExp
                    ...
                    if ~PreStimStarted
                        log.ev.t = [log.ev.t; vbl];
                        log.ev.type = [log.ev.type; 'PreStimStart'];
                        log.ev.info = [log.ev.info; num2str(T)];
                        if eyetracker.do
                ...
                while  ~RespLogged
                    ...
                    if KeyIsDown 
                        ...
                        if strcmp(keys.LastKey,keys.resp{1}) || strcmp(keys.LastKey,keys.resp{2})
                            log.ev.t = [log.ev.t; keys.secs];
                            log.ev.type = [log.ev.type; 'PreStimResponse'];
                            log.ev.info = [log.ev.info; keys.LastKey];
                            RespLogged = true;
                        end
                    end
                end
            ...
            log.ev.t = [log.ev.t; secs];
            log.ev.type = [log.ev.type; 'GapStart'];
            log.ev.info = [log.ev.info; num2str(T)];
            if eyetracker.do
            ...
            while vbl-StimStarted < trialtime.StimT  && ~StopExp
                    ...
                    switch stim(ss).type
                        ...
                        case 'dots'
                            ...
                            Screen('DrawDots',monitor.w,...
                                ...
                                stim(ss).dotcol,monitor.center,0)
                            Screen('DrawTexture', monitor.w, ....
                                StimMask, [], drect, []);
                        case 'image'
                            drect = [0 0 StimSizePix(1) StimSizePix(2)];
                ...
                if ~StimStarted
                    % log
                    log.ev.t = [log.ev.t; vbl];
                    log.ev.type = [log.ev.type; 'StimStart'];
                    log.ev.info = [log.ev.info; num2str(T)];
                    if eyetracker.do
                ...
                elseif keys.LogNewKey
                    log.ev.t = [log.ev.t; keys.secs];
                    log.ev.type = [log.ev.type; 'KeyPress'];
                    log.ev.info = [log.ev.info; keys.LastKey];
                end
            ...
            % done with stimulus
            log.ev.t = [log.ev.t; vbl];
            log.ev.type = [log.ev.type; 'StimStop'];
            log.ev.info = [log.ev.info; num2str(T)];
            if eyetracker.do
    ...
    save(fullfile(RunPath, log.fld, log.Label, 'log', 'settings'));
    ...
    vbl = Screen('Flip', monitor.w);
    pause(5); % pause a few seconds
catch
    %% Error handling
    sca; ShowCursor;
    psychrethrow(psychlasterror);
    Screen('LoadNormalizedGammaTable', monitor.w, OLD_Gamtable);
end
...
%% Repeating functions
    ...
    function SetMonitors(state)
        ...
        switch state
            ...
            case 'extended'
                system("xrandr --output DP-1 --right-of DP-0") % set monitors extended
        end
